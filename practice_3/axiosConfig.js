...
api.interceptors.request.use((config) => {
    var _a;
    console.log(`[Axios Request Interceptor] ${(_a = config.method) === null || _a === void 0 ? void 0 : _a.toUpperCase()} 요청 보냄: ${config.url}`);
    return config;
}, (error) => {
    console.error(`[Axios Request Interseptor] 요청 에러: ${error.message}`);
...
api.interceptors.response.use((response) => {
    ...
    console.log(`[Axios Response Interseptor] 응답 받음: ${response.config.url} (상태: ${response.status})`);
    return response;
}, (error) => {
    var _a;
    // 2xx 범위 밖의 에러 응답이 왔을 때 (4xx, 5xx 등)
    console.error(`[Axios Response Interceptor] 응답 에러 (${(_a = error.response) === null || _a === void 0 ? void 0 : _a.status}):`, 
    ...
    if (error.response) {
        switch (error.response.status) {
            case 401:
                console.error('  -> Unauthorized: 인증 실패! 로그인 페이지로 리다이렉트!');
        ...
        console.error('  -> No Response: 서버로부터 응답을 받지 못했습니다. 네트워크 확인 필요!');
    }
    else {
        // 요청 설정 등 그 외의 에러
